...
export const ADMIN_COOKIE = process.env.NODE_ENV === "production" ? "__Host-vexdou_admin" : "vexdou_admin";
...
function sign(payload: string): string { return createHmac("sha256", getSecret()).update(payload).digest("base64url"); }

export function createAdminSession(): string {
  const payload = JSON.stringify({ exp: Math.floor(Date.now() / 1000) + SESSION_SECONDS, nonce: randomBytes(16).toString("hex") });
